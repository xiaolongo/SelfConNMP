...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.bn1 = torch.nn.BatchNorm1d(8 * dim)
        self.fc1 = Linear(8 * dim, 8 * dim)

        self.conv2 = GATConv(8 * dim, dim, heads=heads, concat=True)
        self.bn2 = torch.nn.BatchNorm1d(8 * dim)
        self.fc2 = Linear(8 * dim, 8 * dim)

        self.conv3 = GATConv(8 * dim, dim, heads=heads, concat=True)
        self.bn3 = torch.nn.BatchNorm1d(8 * dim)
        self.fc3 = Linear(8 * dim, 8 * dim)

        self.conv4 = GATConv(8 * dim, dim, heads=heads, concat=True)
        self.bn4 = torch.nn.BatchNorm1d(8 * dim)
        self.fc4 = Linear(8 * dim, 8 * dim)

        self.conv5 = GATConv(8 * dim, dim, heads=heads, concat=True)
        self.bn5 = torch.nn.BatchNorm1d(8 * dim)
        self.fc5 = Linear(8 * dim, 8 * dim)

        self.lin2 = Linear(8 * dim, 1)
