...
# writer = SummaryWriter('runs/reddit_m/dsc_reddit_m')
...
class Net(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch):
        ...
        x3_g = self.fc2(x0_g + x1_g + x2_g + x3_g)
        # x3_g = self.fc3(x3_g)

        x4 = self.conv4(x3, edge_index)
        x4_g = global_add_pool(x4, batch)
        x4_g = self.fc2(x0_g + x1_g + x2_g + x3_g + x4_g)

        x5 = self.conv5(x4, edge_index)
        x5_g = global_add_pool(x5, batch)
        x5_g = self.fc2(x0_g + x1_g + x2_g + x3_g + x4_g + x5_g)

        x = F.dropout(x5_g, p=0.5, training=self.training)
        x = self.lin(x)
